...
class SchedulerService {
  ...
  async pingUrl(url) {
    ...
    const host = this.domain_from_url(url)
    const response = await ping.promise.probe(host, this.config)
  ...
  async scheduleTask(cron, url) {
    //We schedule the task only if the host is alive
    let hostIsAlive = await this.pingUrl(url)
    if (hostIsAlive) {
